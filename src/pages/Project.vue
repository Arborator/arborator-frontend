...
export default {
  ...
  methods: {
    ...
    resetParsingProgress() {
      clearInterval(this.parser.timer);
    ...
    bootParserStop() {
      api
        .removeParseFolder(this.$route.params.projectname, this.parser.sha512Fdname)
        .then((response) => {
    ...
    getProgress(fdname) {
      ...
      api
        .bootParserResults(this.$route.params.projectname, this.parser.param.type, fdname)
        .then((response) => {
    ...
    bootParserDefault() {
      ...
      api
        .bootParserDefault(samplenames, this.$route.params.projectname)
        .then((response) => {
          this.table.selected = [];
      ...
      if (this.parser.parsing) {
        this.parser.timer = setInterval(() => {
          setTimeout(this.getProgress(this.parser.sha512Fdname), 10);
        }, 20 * 1000);
      }
    },
    bootParserCustom() {
