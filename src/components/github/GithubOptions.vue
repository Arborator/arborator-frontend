        ...
        <q-list>
            <q-item :disable="changesNumber == 0" clickable v-close-popup @click="isShowCommitDialog = true">
                ...
                <q-item-section>
                    ...
                    <q-item-label v-if="changesNumber > 0" caption> {{ $t('github.commitNotif[0]') }} {{ changesNumber }} <span
                            v-if="changesNumber === 1">{{ $t('github.commitNotif[1]') }}</span><span
                            v-else>{{ $t('github.commitNotif[2]') }}</span></q-item-label>
