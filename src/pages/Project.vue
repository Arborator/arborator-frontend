<template>
  <q-page id="container" class="custom-frame1">
    <q-card flat style="max-width: 100%" class="custom-frame1">
      <q-card-section no-shadow	class="project-header">
        <q-img class="project-image" :src="cleanedImage" basic>
          <div class="absolute-bottom text-h6" style="padding: 6px">
            ...
            <q-btn v-else flat round :color="$q.dark.isActive ? 'primary' : ''" icon="settings"
      ...
      <q-card-section v-if="isShowLexiconPanel">
        ...
        <LexiconMain :sample-id="table.selected" />
      </q-card-section>
          ...
          <template #top="props">
            ...
            <q-btn-group flat>
              ...
              <div v-if="isAllowdedToSync && githubSynchronizedRepo && !isDeleteSync">
                <GithubOptions :projectName="projectName" :repositoryName="githubSynchronizedRepo" :key="reload"
                  @remove="reloadAfterDeleteSynchronization" @pulled="loadProjectData" />
                <q-tooltip content-class="text-white bg-primary"> {{ $t('projectView.tooltipSynchronizedProject') }}
                  {{ githubSynchronizedRepo }}</q-tooltip>
              </div>
            </q-btn-group>
            <!-- End of Options bar -->
