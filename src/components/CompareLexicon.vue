    ...
    >
      ...
      <template v-slot:body-cell="props">
        <td
          v-if="
            props.row.type ==
            'In the two dictionaries with the same information'
          "
        ...
        <td
          v-else-if="
            props.row.type ==
            'In the two dictionaries with different information'
          "
        ...
        <td
          v-else-if="
            props.row.type ==
            'Identical form in both dictionaries with different information'
          "
          style="background: orange"
        >
          {{ props.value }}
        ...
        <td
          v-else-if="props.row.type == 'Only in the old dictionary'"
          style="background: #7ef0cc"
        >
          {{ props.value }}
        </td>
        <td
          v-else-if="props.row.type == 'Only in the imported dictionary'"
          style="background: #faa5cb"
        >
          {{ props.value }}
        </td>
      </template>
    ...
    changeLexicon() {
      ...
      if (this.table.selected.length == 0) {
        ...
        for (let i = 0; i < this.table.selected.length; i++) {
          if (
            this.table.selected[i]["toChange"] == "_" ||
            !this.table.selected[i]["toChange"]
          ) {
            this.featCheck = true;
          }
        }
      ...
      if (this.featCheck == false) {
        for (let i = 0; i < this.table.selected.length; i++) {
          if (this.table.selected[i]["features"] == "_") {
            this.currentinfo =
              this.table.selected[i]["form"] +
              " " +
              this.table.selected[i]["lemma"] +
              " " +
              this.table.selected[i]["POS"] +
              " " +
              this.table.selected[i]["gloss"] +
              " _";
          } else {
            this.currentinfo =
              this.table.selected[i]["form"] +
              ...
              this.table.selected[i]["gloss"] +
              " " +
              this.table.selected[i]["features"];
          }
