      ...
      <q-card>
        <q-bar class="bg-primary text-white">
          <div class="text-weight-bold">
            Select a relation going from "{{ snap.currentword }}" to "{{
              snap.currentdepword
            }}"
          </div>
          <q-space />
        ...
        <q-card-actions>
          ...
          <q-btn
            color="primary"
            @click="onchangerel(options.extendedrel)"
            label="Ok"
            v-close-popup
            style="width: 35%; margin-left: auto; margin-right: auto"
          />
        ...
        <q-card-actions align="around">
          ...
          >
            <q-tooltip
              ...
              >⚠ Changing tokens breaks the comparability of different
              annotations of the same sentence!</q-tooltip
            >
          </q-btn>
        </q-card-actions>
      </q-card>
  ...
  methods: {
    ...
    up(dirty, redo) {
      ...
      if (this.snap.treedata != null) {
        ...
        this.$emit("update-conll", {
          ...
          redo: redo,
          user: this.user,
        }); //emits to parent the id of the tree and a bunch of other stuff
      }
    },
    ...
    start(conllStr, matches, id) {
      ...
      if (this.user in matches) {
        var usermatches = matches[this.user];
      } else {
      ...
      this.up(false, false);
      this.snap.metal = [];
      for (let a in this.snap.treedata.META) {
        this.snap.metal.push({ a: a, v: this.snap.treedata.META[a] });
      }
    ...
    onMetaDialogOk() {
      ...
      if (fun) {
        console.log(
          "the conf.annotationFeatures should be updated via a flask function"
        );
      } // TODO!
    ...
    openFeatureDialog(
      ...
      for (let a in snapfeats) {
        this.featTable.featl.push({ a: a, v: snapfeats[a] });
      }
      this.featTable.miscl = [];
      for (let a in snapmisc) {
        this.featTable.miscl.push({ a: a, v: snapmisc[a] });
      }
    ...
    featureUpdateNeeded() {
      ...
      for (let r of this.featTable.miscl) {
        ...
        if (!cs.includes(r.v)) {
          if (cs == "String") {
            continue;
          } else if (cs == "Number" && parseFloat(r.v) == r.v) {
            continue;
          }
          return true;
        }
    ...
    openTokenDialog(b, e, t) {
      ...
      var spa = Object.values(this.snap.treedata.tree).map(({ MISC }) =>
      ...
      if (outts) {
        this.snap.currentword = sentence.substring(
          outts[0].b,
    ...
    onTokenDialogOk() {
      ...
      var treedata = arboratorDraft.replaceNodes(
        ...
        ttokl.length
          ? this.snap.tokidsequence[0]
          : this.snap.tokidsequence[0] - 1,
        ttokl
      );
      this.$emit("meta-changed", this.snap.treedata.META);
      this.up(true, false);
...
.vs-dark .vs__dropdown-option--highlight {
  background: #501d7d;
  color: white;
}
...
.vs-light .vs__dropdown-option--highlight {
  background: #501d7d;
  color: white;
}
</style>