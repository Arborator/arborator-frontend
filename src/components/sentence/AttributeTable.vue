        ...
        <q-tr :props="props">
          ...
          <q-td key="v" :props="props">
            ...
            >
              ...
              <template v-if="prepend != undefined" v-slot:prepend>{{
                props.row.join
              }}</template>
            </q-select>
          </q-td>
          <q-td v-if="modifiable == 'true'" key="actions" :props="props">
            ...
            >
              <q-tooltip :delay="300"
                >Erase the attribute {{ props.row.a }}</q-tooltip
              >
            </q-btn>
          </q-td>
        </q-tr>
...
export default {
  ...
  methods: {
    ...
    thisdate(timestamp) {
      ...
      if (diff == 0)
        diffstr =
          date.getDateDiff(now, parseInt(timestamp), "hours") + " hours ago: ";
      else if (diff > 365) diffstr = "a long time ago: ";
      else diffstr = diff + " days ago: ";
      return (
        diffstr + date.formatDate(parseInt(timestamp), "YYYY-MM-DD HH:mm:ss")
      );
    },
