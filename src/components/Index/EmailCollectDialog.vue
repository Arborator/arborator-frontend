  ...
  <q-dialog v-model="emailCollectDialog" persistent>
    <q-card style="min-width: 50vw">
      <q-form @submit="onSubmitEmail">
        ...
        <q-card-section class="text-body2 text-weight-light text-justify" :class="$q.dark.isActive ? 'text-white' : 'text-blue-grey-10'">
          {{ $t('homepage.textGetUserEmailDialog[0]') }}
        </q-card-section>
        <q-card-section class="text-body2 text-weight-light text-justify q-pt-none" :class="$q.dark.isActive ? 'text-white' : 'text-blue-grey-10'">
          {{ $t('homepage.textGetUserEmailDialog[1]') }}
        </q-card-section>
        <q-card-section class="text-body2 text-weight-light text-justify q-pt-none" :class="$q.dark.isActive ? 'text-white' : 'text-blue-grey-10'">
          {{ $t('homepage.textGetUserEmailDialog[2]') }}
        </q-card-section>
        <q-card-section>
          <q-input v-model="email" :disable="notShareEmail == true" type="email" label="Email" :rules="[emailRules]" />
