...
export function conllToJson(sentenceConll) {
    ...
    nodes.forEach((nodeline, id) => {
        ...
        if (el > 1) {
            if (!(el in CONLL_OPTIONS.conlls) && el > 10)
                el = 10;
            if (el > 4)
                id = elements[CONLL_OPTIONS.conlls[el]["ID"]];
            else if (elements[CONLL_OPTIONS.conlls[el]["FORM"]] != "_")
                ID++;
            ...
            if (tokids.length == 1) {
                ...
                var tokenJson = {
                    ID: parseInt(id),
                    FORM: form,
                    LEMMA: elements[CONLL_OPTIONS.conlls[el]["LEMMA"]],
                    UPOS: elements[CONLL_OPTIONS.conlls[el]["UPOS"]],
                    XPOS: "",
                    FEATS: {},
                    HEAD: parseInt(elements[CONLL_OPTIONS.conlls[el]["HEAD"]]),
                    DEPREL: elements[CONLL_OPTIONS.conlls[el]["DEPREL"]],
                    DEPS: {},
                    MISC: {},
                };
                // // tokenJson["ID"] = parseInt(id);
                // tokenJson.ID = 1;
                // tokenJson["FORM"] = form;
                // tokenJson["LEMMA"] = elements[CONLL_OPTIONS.conlls[el]["LEMMA"]];
                // tokenJson["UPOS"] = elements[CONLL_OPTIONS.conlls[el]["UPOS"]];
                // tokenJson["HEAD"] = parseInt(elements[CONLL_OPTIONS.conlls[el]["HEAD"]]);
                // tokenJson["DEPREL"] = elements[CONLL_OPTIONS.conlls[el]["DEPREL"]];
                // tokenJson["FEATS"] = {};
                // tokenJson["DEPS"] = {};
                // tokenJson["MISC"] = {};
                treeJson[parseInt(id)] = tokenJson;
                ...
                if (el == 10) {
                    ...
                    treeJson[id]["FEATS"] = analyzeFeaturestring(elements[CONLL_OPTIONS.conlls[el]["FEATS"]], "=", "|");
                    treeJson[id]["DEPS"] = analyzeFeaturestring(elements[CONLL_OPTIONS.conlls[el]["DEPS"]], ":", "|");
                    treeJson[id]["MISC"] = analyzeFeaturestring(elements[CONLL_OPTIONS.conlls[el]["MISC"]], "=", "|");
                }
            }
    ...
    words.forEach((word, i) => {
        sentence.push(word);
        if (!("SpaceAfter" in treeJson[i + 1]["MISC"] &&
...
export function getSentenceTextFromJson(treeJson) {
    ...
    var spa = Object.values(treeJson).map(({ MISC }) => "SpaceAfter" in MISC && MISC.SpaceAfter == "No" ? 0 : 1);
    var sentence = "";
