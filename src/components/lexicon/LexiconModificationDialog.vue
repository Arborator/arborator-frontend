  ...
  <q-dialog :model-value="isShowLexiconModification" @update:model-value="hideLexiconModificationDialog()">
    <q-card style="min-width: 500px;">
      ...
      <q-card-section class="q-gutter-md">
        <q-input 
          v-if="features.includes('form')" 
          outlined 
          v-model="formattedItem.form" 
          label="FORM" 
        />
        <q-input 
          v-if="features.includes('lemma')" 
          outlined 
          v-model="formattedItem.lemma" 
          label="LEMMA" 
        />
        <q-select
          v-if="features.includes('upos')"
          outlined 
          v-model="formattedItem.upos" 
          :options="annotationFeatures.UPOS"  
          label="Upos">
        </q-select>
        <q-input 
          v-if="features.includes('Gloss')" 
          outlined 
          v-model="formattedItem.gloss" 
          label="Gloss"
         />
      </q-card-section>
      <q-separator />
