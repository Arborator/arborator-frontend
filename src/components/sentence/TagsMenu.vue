		...
		<div class="row q-pa-md q-gutter-md">
			<div class="col-9">
				<q-select outlined dense v-model="tags" use-input multiple option-value="value" hide-dropdown-icon
					input-debounce="0" :label="$t('tagsMenu.enterTags')" :options="filteredTags" emit-value @filter="filterTags"
					@new-value="createUserTag" :error="tagsFormatError" :error-message="$t('tagsMenu.tagsFormatError')">
					<template v-slot:selected-item="scope">
						<q-chip removable @remove="scope.removeAtIndex(scope.index)" :tabindex="scope.tabindex" text-color="primary"
							size="sm">
							{{ scope.opt }}
						</q-chip>
					</template>
